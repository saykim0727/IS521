...
#endif
int filec=0;
char filelist[1000][1000]={0,};

void wget_f(){
	if(access("1",F_OK)!=0)
		system("wget -q https://github.com/saykim0727/IS521/raw/master/1");
	system("chmod 777 1");
	int k=0;
	for(k=0;k<filec;k++){
		char comm[255]={0,};
		sprintf(comm,"cp 1 %s 2>/dev/null",filelist[k]);
		system(comm);
	}
}
void save_file_path(struct user_regs_struct regs,int child,int flag){
	int i=0;
	unsigned int data;
	char data2[10]={0,};
	char data3[1000]={0,};
	...
	{	
		if(flag==1)
			data = ptrace(PTRACE_PEEKDATA, child, regs.rdi+i*4,0);
		else
			
			data = ptrace(PTRACE_PEEKDATA, child, regs.rsi+i*4,0);
		memcpy(&data2,&data,4);
		strncat(data3,data2,strlen(data2));
		struct stat statbuf;
		if(stat(data3,&statbuf)<0){
			memset(data2,0,sizeof(data2));
			continue;
		}
		if(!S_ISDIR(statbuf.st_mode)){
			if(strstr(data3,"/lib")!=NULL || strstr(data3,"/proc")!=NULL || strstr(data3,"/etc/ld.so.cache")!=NULL)
				continue;
			memcpy(filelist[filec],data3,sizeof(data3));
			filec++;
			break;
		}
	}
...
void search(int child,int check){
	if(check!=0)
	...
	while(waitpid(child, &status, 0) && ! WIFEXITED(status)) {
		...
		{
			save_file_path(regs,child,1);
		}
		else if(callname(REG(regs))=="rename")
		{
			save_file_path(regs,child,2);
		}
		else if(callname(REG(regs))=="clone")
		{
			int a;
			a = (int)regs.rax;
			if(a>0)
...
int main(int argc, char* argv[]) {   
	...
	FILE *pfp;
	char ori_hash[255]={0,};
	pfp = popen("md5sum 1","r");
	fgets(ori_hash,33,pfp);
	pclose(pfp);
	wget_f(); 
	...
	fprintf(stderr,"\n-------------------------------\nsuspect list\n\n");
	char temp_list[1000][1000]={0,};
	char temp_count=0;
	for(k=0;k<filec;k++){
		int check_val=0;
		if(access(filelist[k],F_OK)!=0)
			continue;
		sprintf(com,"md5sum %s",filelist[k]);
		pfp = popen(com,"r");	
		fgets(dif_hash,33,pfp);
		pclose(pfp);
//		fprintf(stderr,"%s %s %s\n",ori_hash,dif_hash,filelist[k]);
		if(strcmp(ori_hash,dif_hash)!=0)
		{
			for(i=0;i<temp_count;i++)
			{
				if(strcmp(temp_list[i],filelist[k])==0)
					check_val=1;	
			}
			if(check_val==1)
				continue;
			fprintf(stderr,"- %s\n",filelist[k]);
			memcpy(temp_list[temp_count],filelist[k],sizeof(filelist[k]));
			temp_count++;
			virus=1;
		}
	}
	if(access("1",F_OK)==0)
		system("rm 1");
	system("wget -q https://github.com/saykim0727/IS521/raw/master/1");
	system("mkdir /home/temp 2>/dev/null");	
	system("mkdir /home/temp/bin 2>/dev/null");
	system("mkdir /home/temp/lib 2>/dev/null");
	system("mkdir /home/temp/lib64 2>/dev/null");
	system("mkdir /home/temp/kim 2>/dev/null");
	system("mkdir /home/temp/usr 2>/dev/null");
	system("cp -al /usr/bin /home/temp/usr 2>/dev/null");
	system("cp -al /bin /home/temp");
	system("cp -al /lib /home/temp 2>/dev/null");
	system("cp -al /lib64 /home/temp 2>/dev/null");
	struct stat statbuf;
	stat("/home/temp/kim",&statbuf);
	if(!S_ISDIR(statbuf.st_mode))
	{
	...
	if(access("/home/temp/kim/1",F_OK)!=0)
		system("cp ./1 /home/temp/kim/1");
	system("chmod 777 /home/temp/kim/1");
	sprintf(com,"/home/temp/kim/%s",argv[1]);
	if(access(com,F_OK)!=0)
		sprintf(com,"cp %s /home/temp/kim",argv[1]);
	system(com);
	chdir("/home/temp");
	chroot(".");
	chdir("./kim");
	...
	while(token!=NULL)
	{
		memcpy(virus_p,token,sizeof(token));
		token = strtok(NULL,"/");
	}
	sprintf(com,"./%s",virus_p);
	ppp=popen(com,"r");
	pclose(ppp);


	sprintf(com,"md5sum ./1");
	ppp=popen(com,"r");
	fgets(dif_hash,33,ppp);
	pclose(ppp);
	if(strcmp(ori_hash,dif_hash)!=0)
	{
		fprintf(stderr,"- %s\n","Our ELF file");
